...
let RolesGuard = class RolesGuard {
    canActivate(context) {
        const requiredRoles = this.reflector.getAllAndOverride(_decorators.ROLES_KEY, [
            context.getHandler(),
            context.getClass()
        ]);
        ...
        if (!hasRole) {
            throw new _common.ForbiddenException(`Access denied. Required roles: ${requiredRoles.join(', ')}`);
        }
        return true;
    }
