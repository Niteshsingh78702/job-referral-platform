...
let EmployeeService = class EmployeeService {
    ...
    async getProfile(userId) {
        ...
        if (!employee) {
            throw new _common.NotFoundException('Employee profile not found. Please ensure you registered as an Employee.');
        }
        // Get current tier
    ...
    async getDashboardStats(userId) {
        ...
        const paidEarnings = await this.prisma.employeeEarning.aggregate({
            where: {
                employeeId: employee.id,
                status: _constants.EarningStatus.PAID
            },
            _sum: {
                amount: true,
                bonusAmount: true
        ...
        const pendingEarnings = await this.prisma.employeeEarning.aggregate({
            where: {
                ...
                status: {
                    in: [
                        _constants.EarningStatus.PENDING,
                        _constants.EarningStatus.ELIGIBLE
                    ]
                }
            },
            _sum: {
        ...
        return {
            ...
            pendingReferral: referralStats.find((s)=>s.status === _constants.ReferralStatus.PENDING)?._count || 0,
            confirmedReferral: referralStats.find((s)=>s.status === _constants.ReferralStatus.CONFIRMED)?._count || 0,
            availableReferrals,
            thisMonthReferrals,
            totalEarnings: (earningsStats._sum.amount || 0) + (earningsStats._sum.bonusAmount || 0),
            paidEarnings: (paidEarnings._sum.amount || 0) + (paidEarnings._sum.bonusAmount || 0),
            pendingEarnings: (pendingEarnings._sum.amount || 0) + (pendingEarnings._sum.bonusAmount || 0),
    ...
    async getAvailableReferrals(userId, search) {
        ...
        if (!employee.isVerified) {
            throw new _common.ForbiddenException('Your employee account is not verified yet. Please wait for verification.');
        }
        ...
        return referrals.map((ref)=>({
                ...ref,
                potentialEarning: ref.JobApplication.Job.referralFee * commissionRate / 100 + bonusPerReferral,
    ...
    async confirmReferral(userId, applicationId) {
        ...
        if (!employee.isVerified) {
            throw new _common.ForbiddenException('Your employee account is not verified');
        }
        ...
        if (referral.employeeId) {
            throw new _common.BadRequestException('Referral already claimed by another employee');
        }
        // Verify company match
        if (employee.companyName.toLowerCase() !== referral.JobApplication.Job.companyName.toLowerCase()) {
            throw new _common.ForbiddenException('Cannot refer for a different company');
        }
        ...
        const earningAmount = referral.JobApplication.Job.referralFee * commissionRate / 100;
        // Perform transaction
        const result = await this.prisma.$transaction(async (tx)=>{
            ...
            const earning = await tx.employeeEarning.create({
                data: {
                    ...
                    status: _constants.EarningStatus.PENDING,
    ...
    async getEarnings(userId, filters) {
        ...
        const [earnings, total, summary] = await Promise.all([
            ...
            this.prisma.employeeEarning.count({
                where
            }),
    ...
    async getLeaderboard(userId, period = 'all') {
        ...
        const employees = await this.prisma.employee.findMany({
            ...
            orderBy: period === 'month' ? undefined : {
        ...
        let leaderboard = employees.map((emp, index)=>({
                ...
                referralCount: period === 'month' ? emp.Referral?.length || 0 : emp.referralCount,
        ...
        if (period === 'month') {
            leaderboard = leaderboard.sort((a, b)=>(b.referralCount || 0) - (a.referralCount || 0)).map((emp, index)=>({
                    ...emp,
                    rank: index + 1
                }));
        }
        ...
        if (!currentUserInLeaderboard) {
            ...
            const higherRanked = await this.prisma.employee.count({
                where: {
                    ...
                    successfulReferrals: {
                        gt: employee.successfulReferrals
                    }
                }
            });
    ...
    async getCurrentTierInternal(employeeId) {
        ...
        const tier = await this.prisma.commissionTier.findFirst({
            where: {
                minReferrals: {
                    lte: employee.successfulReferrals
                },
                isActive: true
            },
            orderBy: {
        ...
        return {
            current: tier,
            next: nextTier,
            referralsToNextTier: nextTier ? nextTier.minReferrals - employee.successfulReferrals : null
        };
    }
    ...
    async markReferralAsHired(referralId) {
        ...
        if (referral.status !== _constants.ReferralStatus.CONFIRMED) {
            throw new _common.BadRequestException('Referral must be confirmed first');
        }
        return this.prisma.$transaction(async (tx)=>{
            ...
            if (referral.EmployeeEarning) {
                await tx.employeeEarning.update({
                    where: {
                        id: referral.EmployeeEarning.id
                    },
                    data: {
                        status: _constants.EarningStatus.ELIGIBLE
                    }
                });
            }
            // Update employee stats
