...
export class AtsResumeService {
    ...
    async uploadAndParse(file: Express.Multer.File) {
        ...
        const debugPath = path.join(process.cwd(), 'tmp', 'debug_raw_text.txt');
        if (!fs.existsSync(path.join(process.cwd(), 'tmp'))) fs.mkdirSync(path.join(process.cwd(), 'tmp'), { recursive: true });
        fs.writeFileSync(debugPath, rawParsed.text, 'utf-8');
        ...
        return { parsedData: improvedData, atsScore: score, suggestions };
    }

    scoreResume(data: ParsedResumeJson) {
    ...
    private buildStructuredJson(rawParsed: any): ParsedResumeJson {
        ...
        return {
            ...
            summary, skills, experience: allExperience, education, certifications,
        };
    }
    ...
    private splitIntoSections(lines: string[]): Record<string, string[]> {
        const sectionPatterns: { name: string; pattern: RegExp }[] = [
            { name: 'summary', pattern: /^(?:professional\s*)?summary|^objective|^profile|^about\s*me|^career\s*(?:summary|objective)|^personal\s*statement|^overview/i },
            { name: 'skills', pattern: /^(?:key\s*|technical\s*|core\s*)?skills|^core\s*competencies|^technologies|^areas?\s*of\s*expertise|^strengths|^proficiencies|^capabilities|^tools\s*(?:\&|and)\s*technologies/i },
            { name: 'experience', pattern: /^(?:professional\s*|work\s*)?experience|^employment|^work\s*history|^career\s*history|^positions?\s*held|^responsibilities|^internships?/i },
            { name: 'education', pattern: /^education|^academic|^qualification|^scholastic|^educational\s*qualifications?|^academics/i },
            { name: 'certifications', pattern: /^certification|^certificate|^licenses?|^professional\s*development|^courses?|^training|^achievements?|^awards?|^honors?|^accomplishments?/i },
            { name: 'projects', pattern: /^(?:key\s*|personal\s*|academic\s*)?projects?|^portfolio/i },
        ];

        const sectionBoundaries: { name: string; lineIdx: number }[] = [];
            ...
            for (const { name, pattern } of sectionPatterns) {
                if (pattern.test(line)) {
                    ...
                    if (!sectionBoundaries.some(b => b.name === name)) {
                        sectionBoundaries.push({ name, lineIdx: i });
                    }
                    break;
                }
            }
    ...
    private extractSkillsFromSections(sections: Record<string, string[]>, rawSkills: string[], fullText: string): string[] {
    ...
    private extractSkillsFromFullText(text: string): string[] {
        const common = [
            'JavaScript', 'TypeScript', 'Python', 'Java', 'C++', 'C#', 'Ruby', 'PHP',
            'React', 'Angular', 'Vue', 'Node.js', 'Express', 'NestJS', 'Django', 'Flask',
            'Spring Boot', 'Spring', 'MongoDB', 'PostgreSQL', 'MySQL', 'Redis', 'Kafka',
            'AWS', 'Azure', 'GCP', 'Docker', 'Kubernetes', 'Jenkins', 'Git', 'GitHub',
            'HTML', 'CSS', 'REST API', 'REST APIs', 'GraphQL', 'Microservices', 'Agile',
            'Hibernate', 'JPA', 'Swagger', 'Postman', 'Linux', 'OAuth2', 'JWT', 'SQL',
            'JUnit', 'Mockito', 'Maven', 'Gradle', 'Log4j2', 'CI/CD', 'Scrum',
            'Terraform', 'Ansible', 'RabbitMQ', 'Elasticsearch',
        ];
    ...
    private extractExperienceFromSection(sections: Record<string, string[]>): ParsedResumeJson['experience'] {
        ...
        const experiences: ParsedResumeJson['experience'] = [];
        ...
        const isBulletLine = (l: string) =>
            /^[•·▪●■]\s?/.test(l) || /^[-–—]\s*[A-Z]/.test(l) || /^\d+[.\\)]\s/.test(l);
        ...
        for (let i = 0; i < expLines.length; i++) {
            ...
            if (hasDate && hasTitle && line.length < 200) {
                ...
                current = { role, company, startDate, endDate, project: '', projectDescription: '', bullets: [] };
            ...
            if (current && !hasDate && line.length > 5 && line.length < 120
                && /^[A-Z]/.test(line) && !hasTitle
                && (i + 1 < expLines.length && titleKeywords.test(expLines[i + 1]) && !isBulletLine(expLines[i + 1]))) {
                ...
                current = { role: subRole, company: parentCompany, startDate: '', endDate: '', project: line, projectDescription: '', bullets: [] };
    ...
    private extractProjectsFromSection(sections: Record<string, string[]>): ParsedResumeJson['experience'] {
        ...
        const projects: ParsedResumeJson['experience'] = [];
        let current: any = null;
    ...
    private extractEducationFromSection(sections: Record<string, string[]>): ParsedResumeJson['education'] {
        ...
        while (i < eduLines.length) {
            ...
            if (hasAnyDate && !hasDegree) {
                ...
                education.push({ institution, degree, field, year, grade });
                this.logger.log(`EDU(1): inst="${institution}", degree="${degree}", year="${year}"`);
            }
            // Pattern 2: Degree on this line (e.g. "B.Tech in CSE, XYZ University — 8.5 CGPA (2024)")
            else if (hasDegree) {
    ...
    // ── Certifications ──
    private extractCertificationsFromSection(sections: Record<string, string[]>): ParsedResumeJson['certifications'] {
        const certLines = sections.certifications || [];
        if (certLines.length === 0) return [];

        const certs: ParsedResumeJson['certifications'] = [];
    ...
    private improveResumeContent(data: ParsedResumeJson): ParsedResumeJson {
        const improved = JSON.parse(JSON.stringify(data)) as ParsedResumeJson;
    ...
    private generateSummary(data: ParsedResumeJson): string {
        const parts: string[] = [];
    ...
    calculateAtsScore(data: ParsedResumeJson): { score: number; suggestions: string[] } {
        let score = 0;
    ...
    private async generateLatexPdf(data: ParsedResumeJson): Promise<Buffer> {
...
`;
        ...
        const fileId = uuidv4();
        const texPath = path.join(tmpDir, `${fileId}.tex`);
        const pdfPath = path.join(tmpDir, `${fileId}.pdf`);
        fs.writeFileSync(texPath, tex, 'utf-8');

        try {
            await execAsync(`pdflatex -interaction=nonstopmode -output-directory="${tmpDir}" "${texPath}"`, { timeout: 30000 });
            return fs.readFileSync(pdfPath);
        } finally {
            for (const ext of ['.tex', '.pdf', '.aux', '.log', '.out']) {
                const p = path.join(tmpDir, `${fileId}${ext}`);
    ...
    private categorizeSkills(skills: string[]): Record<string, string[]> {
        const categories: Record<string, string[]> = {
            'Languages': [], 'Core Backend': [], 'Frameworks & ORM': [],
            'DevOps & Tools': [], 'Testing': [], 'Practices': [], 'Other': [],
        };
        ...
        const mapping: Record<string, string> = {
            ...
            'kotlin': 'Languages', 'sql': 'Languages', 'html': 'Languages',
            'css': 'Languages', 'go': 'Languages', 'rust': 'Languages', 'scala': 'Languages',
            'spring boot': 'Core Backend', 'spring': 'Core Backend',
            'rest api': 'Core Backend', 'rest apis': 'Core Backend',
            'microservices': 'Core Backend', 'oauth2': 'Core Backend', 'oauth': 'Core Backend',
            'redis': 'Core Backend', 'kafka': 'Core Backend',
            'mysql': 'Core Backend', 'postgresql': 'Core Backend',
            'mongodb': 'Core Backend', 'elasticsearch': 'Core Backend',
            'node.js': 'Core Backend', 'node': 'Core Backend',
            'express': 'Core Backend', 'nestjs': 'Core Backend',
            'graphql': 'Core Backend', 'jwt': 'Core Backend',
            'hibernate': 'Frameworks & ORM', 'jpa': 'Frameworks & ORM',
            'react': 'Frameworks & ORM', 'angular': 'Frameworks & ORM',
            'vue': 'Frameworks & ORM', 'django': 'Frameworks & ORM',
            'flask': 'Frameworks & ORM', 'mvc': 'Frameworks & ORM',
            'jenkins': 'DevOps & Tools', 'docker': 'DevOps & Tools',
            'kubernetes': 'DevOps & Tools', 'git': 'DevOps & Tools',
            'github': 'DevOps & Tools', 'postman': 'DevOps & Tools',
            'swagger': 'DevOps & Tools', 'log4j2': 'DevOps & Tools',
            'aws': 'DevOps & Tools', 'azure': 'DevOps & Tools',
            'gcp': 'DevOps & Tools', 'linux': 'DevOps & Tools',
            'terraform': 'DevOps & Tools', 'ansible': 'DevOps & Tools',
            'ci/cd': 'DevOps & Tools', 'maven': 'DevOps & Tools', 'gradle': 'DevOps & Tools',
            'junit': 'Testing', 'mockito': 'Testing', 'jest': 'Testing',
            'pytest': 'Testing', 'selenium': 'Testing',
            'agile': 'Practices', 'scrum': 'Practices',
            'debugging': 'Practices', 'exception handling': 'Practices',
            'api documentation': 'Practices', 'devops': 'Practices',
        };
    ...
    private async generateHtmlPdf(data: ParsedResumeJson): Promise<Buffer> {
