...
export class TokenService {
    ...
    private tokenStore: Map<string, { token: string; expiry: number }> = new Map();
    ...
    private initRedis(): void {
        ...
        if (redisUrl || redisHost) {
            try {
                ...
                this.logger.warn(`Failed to initialize Redis: ${(error as Error).message}. Using in-memory fallback.`);
                this.redis = null;
            }
        } else {
    ...
    private async storeRefreshToken(
        ...
        if (this.redis) {
            try {
                ...
                this.logger.warn(`Redis store failed: ${(error as Error).message}. Using in-memory.`);
    ...
    private storeInMemory(key: string, token: string, expirySeconds: number): void {
