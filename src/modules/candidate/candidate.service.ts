...
export class CandidateService {
  ...
  async deleteApplication(userId: string, applicationId: string) {
    ...
    if (hasSuccessfulPayment) {
      throw new BadRequestException(
        'Cannot delete application with successful payment. Please contact support.'
      );
    }

    const jobTitle = (application as any).Job?.title || 'Unknown';
    const companyName = (application as any).Job?.companyName || 'Unknown';
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (sessionIds.length > 0) {
        ...
        await tx.candidateTestAttempt.deleteMany({ where: { testSessionId: { in: sessionIds } } });
        await tx.testSession.deleteMany({ where: { applicationId } });
      }
      ...
      const referral = await tx.referral.findUnique({ where: { applicationId } });
      if (referral) {
        await tx.employeeEarning.deleteMany({ where: { referralId: referral.id } });
        await tx.referral.delete({ where: { applicationId } });
      }
