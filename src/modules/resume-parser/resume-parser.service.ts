...
export class ResumeParserService {
  private readonly skillKeywords = [
    ...
    'swift', 'kotlin', 'go', 'rust', 'scala', 'perl',
    'react', 'angular', 'vue', 'next.js', 'nuxt',
    'node', 'node.js', 'express', 'nestjs', 'django', 'flask', 'spring', 'spring boot',
    'mongodb', 'postgresql', 'mysql', 'redis', 'elasticsearch', 'dynamodb', 'cassandra',
    'aws', 'azure', 'gcp', 'docker', 'kubernetes', 'jenkins', 'terraform', 'ansible',
    'git', 'github', 'gitlab', 'bitbucket',
    'linux', 'unix', 'windows',
    'html', 'css', 'sass', 'less', 'tailwind', 'bootstrap',
    'rest api', 'rest apis', 'graphql', 'grpc',
    ...
    'junit', 'mockito', 'jest', 'mocha', 'pytest', 'selenium',
    'swagger', 'postman', 'log4j', 'log4j2', 'slf4j',
    ...
    'figma', 'adobe xd',
    'power bi', 'tableau',
    'api documentation', 'debugging', 'exception handling',
  ...
  async parseResume(file: Express.Multer.File): Promise<ParsedResumeData> {
    ...
    try {
      ...
      // Parse plain text files (.txt, .tex, .rtf, etc.)
      else if (
  ...
  private extractSkills(text: string): string[] {
    ...
    this.skillKeywords.forEach((skill) => {
      ...
      if (regex.test(lowerText)) {
        // Capitalize properly
        const properName = skill.split(' ').map(w =>
          w === 'api' || w === 'apis' ? w.toUpperCase() :
            w === 'ci/cd' ? 'CI/CD' :
              w === 'aws' || w === 'gcp' || w === 'jwt' || w === 'sql' || w === 'html' || w === 'css' || w === 'ai' ? w.toUpperCase() :
  ...
  private extractExperience(text: string): {
    ...
    return { years: totalYears, positions: positions.slice(0, 10) };
  }
