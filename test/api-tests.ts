...
async function runAllTests() {
  ...
  await runTest('TC-01: Candidate Signup (Endpoint Exists)', async () => {
    ...
    assert(
      ...
      `Signup endpoint issue: ${res.status}`,
    );
  ...
  await runTest('TC-09: HR Login', async () => {
    ...
    assert(
      ...
      `HR login unexpected: ${res.status}`,
    );
  });
  ...
  await runTest('TC-11: Payment Requires Interview', async () => {
    ...
    const res = await makeRequest(
      'POST',
      `/payments/interview/${fakeInterviewId}/create-order`,
      candidateToken,
    );
  ...
  await runTest('TC-17: Admin Login', async () => {
    ...
    assert(
      [200, 401, 404].includes(res.status),
      `Admin login unexpected: ${res.status}`,
    );
  });
  ...
  await runTest(
    'TC-19: Admin Override Test (if candidate exists)',
    async () => {
      if (!adminToken || !candidateId || !skillBucketId) return;
      const res = await makeRequest(
        'POST',
        '/admin/skill-tests/pass',
        adminToken,
        {
          candidateId,
          skillBucketId,
  ...
  await runTest('TC-20: Fake Interview Payment Blocked', async () => {
    const res = await makeRequest(
      'POST',
      '/payments/interview/fake-id/create-order',
      candidateToken,
    );
