...
async function runMasterE2ETest() {
  ...
  await runStep(1, 'Login as Admin', async () => {
    ...
    if ([200, 201].includes(res.status)) {
      ...
      console.log(
        `  📍 Admin token received: ${adminToken ? 'YES (' + adminToken.substring(0, 20) + '...)' : 'NO'}`,
      );
      console.log(`  📍 Admin role: ${data?.user?.role}`);
    }
    assert(
      [200, 201].includes(res.status),
      `Login failed: ${res.status} - ${JSON.stringify(res.data).substring(0, 100)}`,
    );
  });
  ...
  await runStep(2, 'Navigate to Admin Dashboard', async () => {
    ...
    assert(
      ...
      `Dashboard access failed: ${res.status}`,
    );
  });
  ...
  await runStep(3, 'Get/Create Skill Bucket for Java Backend', async () => {
    ...
    if (res.status === 200 && Array.isArray(buckets) && buckets.length > 0) {
      // Use existing bucket
      const javaBucket = buckets.find(
        (b: any) =>
          b.name?.toLowerCase().includes('java') ||
          b.code?.toLowerCase().includes('java'),
      );
      if (javaBucket) {
    ...
    assert(
      !!skillBucketId || [200, 500].includes(res.status),
      `Skill bucket setup failed: ${res.status}`,
    );
  });

  await runStep(
    4,
    'Create/Verify Test Template with 10 Questions',
    async () => {
      ...
      if (
        ...
        questionsRes.data?.data?.length >= 10
      ) {
  ...
  await runStep(5, 'Verify Question Edit Capability', async () => {
    if (questionIds.length > 0) {
      ...
      assert(
        [200, 403].includes(res.status),
        `Question bank access: ${res.status}`,
      );
    }
  });
  ...
  await runStep(6, 'Verify Question Delete Capability', async () => {
    ...
    assert(
      ...
      `Question stats access: ${res.status}`,
    );
  });
  ...
  await runStep(8, 'Login as HR', async () => {
    ...
    assert(
      [200, 201, 401, 404, 500].includes(res.status),
      `HR login: ${res.status}`,
    );
  });
  ...
  await runStep(11, 'Signup/Login as Candidate', async () => {
    ...
    assert(
      [200, 201, 400, 409].includes(regRes.status),
      `Candidate auth: ${regRes.status}`,
    );
  });
  ...
  await runStep(16, 'Verify Test Question Delivery', async () => {
    ...
    assert(
      [200, 403].includes(res.status),
      `Question delivery check: ${res.status}`,
    );
  });
  ...
  await runStep(18, 'Apply to First Job', async () => {
    if (jobId && candidateToken) {
      ...
      assert(
        [200, 201, 400, 409].includes(res.status),
        `Apply result: ${res.status}`,
      );
    }
  });
  ...
  await runStep(19, 'Verify Multi-Apply Same Skill', async () => {
    ...
    if (res.status === 200 && res.data?.jobs?.length > 1) {
      const secondJob = res.data.jobs.find(
        (j: any) => j.id !== jobId && j.skillBucketId === skillBucketId,
      );
      if (secondJob && candidateToken) {
        const eligRes = await makeRequest(
          'GET',
          `/jobs/${secondJob.id}/apply-eligibility`,
          candidateToken,
        );
        // Should NOT require new test for same skill
        assert(
          [200].includes(eligRes.status),
          `Multi-apply check: ${eligRes.status}`,
        );
      }
    }
  ...
  await runStep(25, 'Verify Payment Required (₹99)', async () => {
    if (candidateToken && interviewId) {
      const res = await makeRequest(
        'POST',
        `/payments/interview/${interviewId}/create-order`,
        candidateToken,
      );
      if (res.status === 201) {
        paymentId = res.data.orderId;
        assert(
          res.data.amount === 9900,
          `Expected ₹99 (9900 paise), got ${res.data.amount}`,
        );
      }
      assert(
        [200, 201, 400, 404].includes(res.status),
        `Payment order: ${res.status}`,
      );
    }
  ...
  await runStep(27, 'Verify Re-Apply Without New Test', async () => {
    if (candidateToken && skillBucketId) {
      ...
      if (jobsRes.status === 200) {
        const anotherJob = (jobsRes.data?.jobs || []).find(
          (j: any) => j.id !== jobId && j.skillBucketId === skillBucketId,
        );
        if (anotherJob) {
          const eligRes = await makeRequest(
            'GET',
            `/jobs/${anotherJob.id}/apply-eligibility`,
            candidateToken,
          );
          // Should allow apply with existing test
          assert(
            [200].includes(eligRes.status),
            `Re-apply eligibility: ${eligRes.status}`,
          );
        }
      }
    }
  ...
  await runStep(30, 'Admin Disable Test (Verify Capability)', async () => {
    if (adminToken && skillBucketId) {
      ...
      assert(
        [200, 403].includes(res.status),
        `Skill bucket access: ${res.status}`,
      );
    }
